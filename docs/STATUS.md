...
- **Issue #30's three-region-only image/bundle freeze and n4 readiness pilot
  ...
  Pilot-first extension phase `bloc-ec2-i30-tr-n10-b8-p1` is also accepted
  from the frozen source/image/BMax-128 inputs. It retained `30/30` successful,
  cross-node-consistent, deadline-met observations across three blocks, with
  total-slot p50 `380.934 ms` and ACS p50 `290.982 ms`; it qualifies for a
  later full continuation but is not p99-eligible. All 11 lifecycle events and
  three controller jobs passed, Terraform destroyed all 45 resources, both
  independent validators and a fresh authenticated absence audit passed, and
  state is empty. Its provider-cache-free, 446-entry checksummed artifact is
  under `results/ec2/bloc-ec2-i30-tr-n10-b8-p1/`.

  The next pilot, `bloc-ec2-i30-tr-n10-b32-p1`, is accepted under the same
  contract with `30/30` successful, consistent, deadline-met observations.
  Total-slot p50 was `751.841 ms` and ACS p50 was `412.075 ms`; it qualifies
  for a later full continuation and is not p99-eligible. All lifecycle/jobs,
  phase/cleanup validators, the 45-resource destroy, fresh authenticated audit,
  ...
  fits at `10/6/6` vCPUs. The previous 22-vCPU same-AZ shortfall is no longer an
  active blocker because that topology is outside issue #30. The conservative
...
- Accepted issue #30 readiness evidence:
  `results/ec2/bloc-ec2-i30-tr-n4-ready-p1/`
- Accepted issue #30 n10/batch-8 pilot evidence:
