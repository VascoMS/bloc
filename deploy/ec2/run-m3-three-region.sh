...
git_commit="$(git -C "$repo_root" rev-parse --short=12 HEAD)"; image_tag="bloc-node:three-region-$git_commit"
if [[ "$plan_only" -ne 1 ]]; then
...
run_phase() (
  ...
  cleanup_phase() {
    ...
    if [[ "$status" -ne 0 ]]; then
      if [[ -f "$phase_root/manifest.json" ]]; then
        ...
        jq -n --arg schema_version bloc-ec2-three-region-phase/v1 --arg experiment_id "$phase_id" --arg status invalid --arg invalid_reason "phase exit $original_status; cleanup exit $status" --arg topology T2-three-region --argjson node_count "$node_count" --arg primary_region "$primary_region" --arg secondary_region "$secondary_region" --arg tertiary_region "$tertiary_region" --arg operator_instance_type "$operator_type" --arg controller_instance_type "$controller_type" --arg cpu_credits "$cpu_credits" --arg docker_image_digest "${phase_digest:-}" --argjson placement "$placement" '{schema_version:$schema_version,experiment_id:$experiment_id,status:$status,invalid_reason:$invalid_reason,topology:$topology,node_count:$node_count,placement:$placement,primary_region:$primary_region,secondary_region:$secondary_region,tertiary_region:$tertiary_region,operator_instance_type:$operator_instance_type,controller_instance_type:$controller_instance_type,cpu_credits:$cpu_credits,docker_image_digest:$docker_image_digest}' >"$phase_root/manifest.json"
      fi
  ...
  jq -n --arg schema_version bloc-ec2-three-region-phase/v1 --arg experiment_id "$phase_id" --arg status complete --arg topology T2-three-region --argjson node_count "$node_count" --arg primary_region "$primary_region" --arg secondary_region "$secondary_region" --arg tertiary_region "$tertiary_region" --arg operator_instance_type "$operator_type" --arg controller_instance_type "$controller_type" --arg cpu_credits "$cpu_credits" --arg docker_image "$runtime_image" --arg docker_image_digest "$phase_digest" --arg git_commit "$git_commit" --arg batch_sizes "$batch_sizes_csv" --argjson warmups "$warmups" --argjson repetitions "$repetitions" --arg eval_timeout "$eval_timeout" --argjson placement "$(jq '.nodes|sort_by(.id)|map({id,region,zone,instance_type})' "$inventory")" --argjson peering_connection_ids "$peerings" '{schema_version:$schema_version,experiment_id:$experiment_id,status:$status,topology:$topology,node_count:$node_count,placement:$placement,primary_region:$primary_region,secondary_region:$secondary_region,tertiary_region:$tertiary_region,peering_connection_ids:$peering_connection_ids,operator_instance_type:$operator_instance_type,controller_instance_type:$controller_instance_type,cpu_credits:$cpu_credits,docker_image:$docker_image,docker_image_digest:$docker_image_digest,git_commit:$git_commit,batch_sizes:($batch_sizes|split(",")|map(tonumber)),warmups:$warmups,repetitions:$repetitions,eval_timeout:$eval_timeout}' >"$phase_root/manifest.json"
