...
3. Run the approved Free Tier-compatible BTE combine-attribution revision with two placements each of `t3.small` and `c7i-flex.large`, explicitly retaining the Intel comparison-host identity in the report; then compare its paired factor ladder with the completed same-AZ and cross-AZ integrated measurements.
